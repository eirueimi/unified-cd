...
| **Template declaring `agentSelector`, `concurrency`, `timeoutMinutes`, `native`, or `finally`** (previously silently ignored under `uses:`) | Remove the field from the template — it never had any effect under `uses:`. If the logic genuinely needs its own agent/run semantics, keep it a `kind: Job` and invoke it with `call:` instead. |
