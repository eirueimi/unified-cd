...
**Appended at the end rather than inline, deliberately: `scenarios/w4-3-pod-start-timeout.md:46` cites `plan:71-84` and `:74` by line, so an in-place banner would have silently invalidated those citations. W3's merge gate (`FINDINGS.md:2112`) requires the propagation, not a particular placement.**
...
**What was NOT propagated: three post-execution corrections to the "Verified code facts" block (`:47-91`; `:93-99` is the separate "Facts NOT established" list and is excluded — the range read `:47-99` until the branch review), each recorded in a runbook and none of which reached this file until now.** This is the recorded-versus-propagated distinction `FINDINGS.md:739` draws.

- **`:53` understates the pod-GC interval finding.** "the only non-test caller passes a literal" — `runPodGC` has **no test call site at all**, so the `<= 0 → time.Minute` guard is unreachable code at HEAD. Corrected at `scenarios/w4-1-podgc-skip-not-delete.md:105-112` (CORRECTION 1); the entry filed on it is `FINDINGS.md:2374`.
- **`:59`'s metrics claim is scoped narrower than what was measured.** It reads "zero hits … **excluding tests**". The measurement is stronger and is an enumeration: `internal/k8sagent/` is **53 files** (flat, no subdirectories; `ls -1` and `find -type f` agree), and `prometheus|promauto|metrics\.|Metric` returns **0** across all 53, **tests included**. Verified twice by execution — `w4-1-podgc-skip-not-delete.md:121-127` (CORRECTION 2) and `w4-2-reuse-swallow.md:844-861` — and both record that a pre-execution figure of **60** was wrong. Note `scenarios/w4-rig.md:56` asserts the same conclusion flatly, with no enumeration and no capture; the 53 is what backs it.
...
- **`:74` states a docs fact that is the opposite of what `docs/configuration.md:456` says.** The plan asserts the boot-rejection/accessor-fallback asymmetry is "Both documented (`docs/configuration.md:456`)". Only the fallback is documented; the boot rejection is documented **nowhere operator-facing**, and `:456` promises the fallback for the very input that refuses to boot. Corrected at `scenarios/w4-3-pod-start-timeout.md:51-87` (CORRECTION 1); it is the wave's W4-3 violation, `FINDINGS.md:2282`.
  **Which half of the pattern this failure belongs to — reclassified at the branch review, against the checkpoint's own interest.** It was counted with the *mechanism* failures. It is not one. `docs/configuration.md:456` **exists** and is the row the plan meant, so the citation resolves; what fails is the **substance** the line is cited for. That makes it a **`file:line` claim failing in substance**, a third failure mode the "line numbers hold, mechanisms fail" dichotomy has no slot for — and filing it under mechanism keeps the "`file:line` half held for a fifth consecutive wave" record **cleaner than the evidence supports**. The honest statement is below.
...
**The pattern, judged rather than restated — and the judgement is weakened at the branch review rather than defended.** W3 refined the rule to "the block's `file:line` claims hold while its *mechanism* claims fail". W4 is the fifth consecutive wave and the `file:line` half held **as far as resolution goes**: every line number, constant and span in the facts block survived checking, with only the `:53` scope adjustment above. **But it did not hold as far as *substance* goes, and `:74` is the counter-example** — its citation resolves to the right row and asserts the opposite of what the row says. **So the fifth-wave verdict is: the `file:line` half held on resolution, failed once on substance, and the campaign should stop reporting the two as one number.** *(Two corrections of scope, both from the branch review: the block is **`:47-91`** — the range this section previously gave, `:47-99`, runs past the facts block into "Facts NOT established" at `:93`, which is a list of declared open questions and cannot fail a check by construction; and `:74` is reclassified out of the mechanism failures above, which leaves the mechanism half at **two** executed refutations plus one belonging to the Task 5 brief rather than to this plan.)* The mechanism half held too, three times over. **W4 adds a third class the earlier waves did not separate: *environment* claims, which failed at a higher rate than mechanism claims did** — four of the block's environment assertions were wrong or moot, and unlike a mechanism error they cost bring-up time rather than a scenario design. The instruction that follows is narrow: an environment claim is checkable in one command at the start of the first task, and W4-0's Step 1 is the model — it spent the first ten minutes checking them and recovered an hour it had budgeted for a TLS workaround that was never needed.
