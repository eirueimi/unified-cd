...
func (m *LocalKeyManager) DecryptKey(_ context.Context, ciphertext []byte) ([]byte, error) {
	if !bytes.HasPrefix(ciphertext, []byte(localKeyPrefix)) {
		return nil, fmt.Errorf("%w: this controller is configured for the %q provider", ErrProviderMismatch, "local")
	}
...
- Produces:
  ...
  - `func (k KeySource) KeyManager() (secrets.KeyManager, string, error)` — returns the manager plus a one-line description for the startup log
...
func TestKeySource_ReadsKeyFile(t *testing.T) {
	ks := KeySource{KeyFile: writeKeyFile(t, testKeyHex)}
	km, desc, err := ks.KeyManager()
	require.NoError(t, err)
	require.NotNil(t, km)
	assert.Contains(t, desc, "key file")
}
...
func TestKeySource_TrimsWhitespaceInKeyFile(t *testing.T) {
	ks := KeySource{KeyFile: writeKeyFile(t, "  "+testKeyHex+"\n")}
	_, _, err := ks.KeyManager()
	require.NoError(t, err)
}

func TestKeySource_RejectsShortKey(t *testing.T) {
	ks := KeySource{KeyFile: writeKeyFile(t, "0102030405")}
	_, _, err := ks.KeyManager()
	require.Error(t, err)
...
func TestKeySource_MissingKeyFileReportsPath(t *testing.T) {
	ks := KeySource{KeyFile: filepath.Join(t.TempDir(), "absent")}
	_, _, err := ks.KeyManager()
	require.Error(t, err)
	assert.Contains(t, err.Error(), "absent")
}

func TestKeySource_DevModeProducesEphemeralKey(t *testing.T) {
	km, desc, err := KeySource{DevMode: true}.KeyManager()
	require.NoError(t, err)
	require.NotNil(t, km)
	assert.Contains(t, strings.ToLower(desc), "ephemeral")
}
...
func TestKeySource_KMSURINotImplementedYet(t *testing.T) {
	_, _, err := KeySource{KMSURI: "hashivault://unified-cd-kek"}.KeyManager()
	require.Error(t, err)
...
func TestKeySource_UnknownKMSSchemeListsSupported(t *testing.T) {
	_, _, err := KeySource{KMSURI: "wat://nope"}.KeyManager()
	require.Error(t, err)
	assert.Contains(t, err.Error(), "hashivault")
}
```
...
// KeyManager resolves the configured source into a secrets.KeyManager. The
// second return value is a short description for the startup log.
func (k KeySource) KeyManager() (secrets.KeyManager, string, error) {
	if err := k.Validate(); err != nil {
		return nil, "", err
	}
	switch {
	case k.KMSURI != "":
		return nil, "", kmsError(k.KMSURI)
	case k.KeyFile != "":
		km, err := localFromFile(k.KeyFile)
		if err != nil {
			return nil, "", err
		}
		return km, "key file " + k.KeyFile, nil
	case k.DevMode:
		km, err := secrets.NewLocalKeyManager(hex.EncodeToString(secrets.GenerateKey()))
		if err != nil {
			return nil, "", err
		}
		return km, "ephemeral development key (secrets will not survive a restart)", nil
	default:
...
func localFromFile(path string) (secrets.KeyManager, error) {
	...
	if runtime.GOOS != "windows" && info.Mode().Perm()&0o077 != 0 {
		fmt.Fprintf(os.Stderr, "warning: key file %s is readable by group or others (mode %#o); chmod 600 it\n",
			path, info.Mode().Perm())
	}
	...
	if len(hexKey) != 64 {
		return nil, fmt.Errorf("key file %s must contain 64 hex characters, got %d", path, len(hexKey))
	}
...
```go
	km, keyDesc, err := eff.KeySource.KeyManager()
	if err != nil {
		slog.Error("encryption key", "error", err)
		os.Exit(1)
	}
	if eff.KeySource.DevMode && eff.KeySource.KeyFile == "" && eff.KeySource.KMSURI == "" {
		slog.Warn("UNIFIED_DEV_MODE is set — using an ephemeral encryption key. " +
			"Secrets stored now cannot be decrypted after a restart. Never use this in production.")
	}
	slog.Info("encryption key loaded", "source", keyDesc)
```
...
func TestSession_RefreshTokenIsNotStoredInPlaintext(t *testing.T) {
	...
	assert.False(t, strings.Contains(string(got.RefreshTokenCT), plaintextRT))
...
func TestSession_RefreshTokenIsBoundToItsSession(t *testing.T) {
	...
	a, err := s.createSessionWithRefreshToken(ctx, "hash-a", "sub-a", "a@example.com", "admin", "token-a", time.Now().Add(time.Hour))
	require.NoError(t, err)
...
func (s *Server) createSessionWithRefreshToken(ctx context.Context, tokenHash, sub, email, role, refreshToken string, expiresAt time.Time) (*store.Session, error) {
	sess, err := s.store.CreateSession(ctx, tokenHash, sub, email, role, nil, nil, expiresAt)
	if err != nil {
		return nil, fmt.Errorf("create session: %w", err)
	}
