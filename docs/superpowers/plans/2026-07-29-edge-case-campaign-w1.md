...
> **Corrected after execution.** As originally written this snippet (and the `mutex-successor` one below) used a top-level `spec.mutex:`, contradicting this plan's own line 19. It has never been a valid field: `Mutex` lives on the `Concurrency` struct (`internal/dsl/types.go:86`) and `dec.KnownFields(true)` (`internal/dsl/parse.go:94`) makes the stray key a hard 400. Task 2 copied the bad form into both payload files (`b112b5c`); W1-4 hit the 400 live and `b3158ce` fixed the assets. Both snippets are corrected here so the plan and the checked-in assets agree.
...
> **⚠ Task 8 was designed against a hypothesis that turned out to be false, and the three bullets below have been corrected after execution.** As originally written, this task's Interfaces line, Part A and Recording line all asserted that "access-token TTL 1h with lazy refresh 15 min before expiry → revocation does NOT bite a live agent quickly", derived a "~45-min blind window" from that, and instructed the executor to *observe* the window rather than test for it. W1-6 disproved it: `agentAuth` (`internal/controller/agent_auth.go:62-77`) re-reads the credential row from Postgres on **every** agent API request, with no server-side credential cache, so the first agent call after the `UPDATE` commits is rejected. The two TTL constants are individually correct but govern only how often the **client** re-exchanges its refresh token — a different quantity from how long a token is *accepted*. Measured agent-observed latency: **4.95s** for `revoke-credentials`, **1.51s** for `disable`, both entirely the agent's own poll cadence (5s cancel poll / 15s heartbeat), not a server-side grace. **The bullets are rewritten below to record what was measured; the fact that the scenario was designed around a false premise is itself the wave's methodological lesson and is deliberately preserved here rather than erased** — see FINDINGS.md carry-forward item 2. Plan line 19 was corrected in the same pass.

**Interfaces:** Consumes `longrun.payload.json`; `unified-cli agent identity revoke-credentials|disable <agent-id>` (run via `docker compose ... run --rm agent-enroll go run -buildvcs=false ./cmd/unified-cli agent identity ...` — the enroll service has the toolchain and UNIFIED_SERVER/TOKEN env). Invariants I6, I7. **Measured (corrected after execution; originally stated as "revocation does NOT bite a live agent quickly"):** revocation is effectively immediate server-side — it bites on the agent's very next request, i.e. within one agent poll interval, 1.5-5.0s measured. The 1h access-token TTL (`internal/controller/api_agent_enrollment.go:21`) and the 15-min refresh lead plus up to 5-min jitter (`internal/agent/credentials.go:27-28,140`) are client-side only and create **no** server-side blind window.

- [ ] **Step 1: Write the runbook:**
  - **Part A (revocation latency — measurement, corrected after execution):** trigger `edge-longrun`; identify claiming agent; `agent identity revoke-credentials <agent-id>`; observe 5 min. **Measured outcome:** the agent keeps *executing* but stops being able to *report* at its very next call — `lastSeenAt` freezes at the last pre-revocation heartbeat and never advances again, log delivery stops at the same instant, and the run is left for the stuck-run reaper. Latency to first observed rejection was **4.95s**, which is the 5s cancel-poll cadence, not a grace period. *(Originally this bullet read: "observe 5 min: the agent keeps executing AND keeps reporting successfully on its cached token (record: heartbeats still accepted, logs still flowing — this is the ~45-min blind window in action). Record as an observation with the exact TTL math (60m TTL − 15m lead) unless something disproves it." Something did disprove it; there is no such window, and the TTL math describes client-side re-exchange only.)*
  ...
  - Recording: run stuck Running forever = major (I1); status/audit contradiction = major (I7); **the revocation latency = observation, and record it prominently — it is the fleet's revocation SLA.** Measured: **1.5-5.0s**, and that is agent poll cadence, not server-side grace — the true server-side latency is zero, so the only residual exposure after an `UPDATE` commits is an in-flight request already past the auth middleware. *(Originally this bullet read: "the 45-min blind window = observation (known TTL design), but record it prominently — it is the fleet's revocation SLA." The SLA framing was right; the number was wrong by roughly three orders of magnitude and in the wrong direction. Note the second-order lesson: a bullet that both asserts a figure and calls it the fleet's SLA invites the executor to transcribe rather than measure it.)*
  - **Also record — this is the part revocation does *not* do:** revocation stops *reporting*, not *execution*. W1-6 Part A measured 162.5s of unfenced post-`Failed` step execution, bounded only by the step's own remaining runtime. `revoke-credentials` is a reporting kill switch, not a containment action.
