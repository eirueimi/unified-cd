...
- No DB schema migration. `logs.step_index` is a bare `integer NOT NULL`; sentinel values are storable today. Sidecar names are derived at read-time from the run's stored spec (`GetRunSpec` + `json.Unmarshal` into `dsl.Spec`), never a new column.
