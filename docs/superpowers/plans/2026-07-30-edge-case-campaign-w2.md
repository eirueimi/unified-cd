...
- **Eleven leader-elected background jobs**, all session-level `pg_try_advisory_lock` via `AcquireAdvisoryLock` (`postgres.go:1425-1450`), wired in `cmd/controller/main.go:367-449`:
  ...
  | Audit retention | `0x61756474` | 1h | :420 — ~~no-op unless `--audit-retention-days > 0`~~ **CORRECTED after W2-1 execution: wired by default, no flag needed** (see below) |
  | Run retention | `0x7272746E` | 1h | :426 (no-op unless configured — **re-verified by W2-1 and correct as written**: `runRetentionDaysDefault()` returns 0 on empty env (`main.go:47-58`) and `RunRunRetention` returns before creating its ticker when `retentionDays <= 0` (`run_retention.go:26-28`); `test/ha` boot-logs `"run retention disabled (keep forever)"`) |
...
- **Three background jobs have NO leader election** and run on every replica: `RunGitResolver` (`main.go:437-445`, 200ms tick, `resolveGitPendingRuns` at `scheduler.go:290` acquires no lock — and its `failureBackoff` comment at `scheduler.go:271-274` presumes a leader it does not elect); the inline `DeleteStaleAgents` loop (`main.go:382-398`, 1m tick, 5m threshold); the inline `DeleteExpiredOIDCStates` loop (`main.go:368-381`, 10m tick).
  - **CORRECTED after W2-1 execution — the absence of a lock is a violation for at most ONE of the three, not all three.** This plan's Task 2 Recording rule ("Absence of leader election on the three unlocked jobs = **violation**, severity by job") was wrong for two of them. `docs/high-availability.md:91-92` lists both **Git resolver** (`none (idempotent)` / "Runs on all replicas. `git://` URI resolution is idempotent and harmless if duplicated") and **OIDC state cleanup** (`none (idempotent)` / "Runs on all replicas") in the arbitration table, so for those two the missing lock **is** the documented design and confirming it is a null result — a violation there requires demonstrating the "harmless if duplicated" claim itself false. Only **`DeleteStaleAgents`** is absent from that table and from every other arbitration statement in `docs/` (`:374` and `:443` describe its threshold but never its arbitration); that is a documentation **gap**, not a contradiction, and W2-1 filed it as a minor observation. Later tasks: check the arbitration table before assuming an unlocked job is undocumented.
...
- **ESTABLISHED by W2-1 execution — the ten per-tick jobs have NO leader stickiness at all, so each sweep runs up to N times per nominal interval on N replicas.** Each of the ten calls `AcquireAdvisoryLock` at the top of its per-tick helper and `defer release()` before that helper returns, so the key is free again ~1ms later and whichever replica ticks next simply takes it; no `release != nil` state is carried between ticks and the winner changes many times per minute. Measured on the 3-replica `test/ha` rig over a 573.4s idle window: **2.15-2.40 acquisitions per nominal interval** (stuck-run 2.15, queued-run 2.20, AppSource sync 2.25, AppSource reconciler 2.25, approval reaper 2.40 — `w2-1/lock-analysis-idle.txt`). The model predicts **up to N** (holds are 0-10ms against a 30s tick, so independently-phased replicas essentially never collide); the rig fell short of 3.00 only because `controller2` and `controller3` booted **0.7ms** apart (`w2-1/controller-boot.txt:7,12`) leaving their tickers permanently sub-millisecond aligned, so one of that pair always loses. **Consequence for later tasks: any reaper-latency or DB-load budget must use the effective cadence `interval / N`, not `interval`** — e.g. the stuck-run reaper swept every ~13.3s here, not 30s. This tightens reap latency (so a boundary test that budgets a full 30s tick will over-wait) and multiplies per-sweep query load by N. Affects W2-3 (Task 4) and W2-4 (Task 5) directly.
- **Ten of eleven jobs are silent both when not leader and when idle** (`if release == nil { return }` with no log; all summary lines guarded by `if n > 0`). Leader exclusion therefore cannot be verified from logs in the idle case.
  - **CORRECTED after W2-1 execution — `pg_locks` cannot census ten of the eleven keys.** The plan's Task 2 instruction ("`SELECT objid, pid, granted FROM pg_locks WHERE locktype='advisory'` … expect exactly one holder per key") is only usable for the **scheduler**. What `pg_locks` **CAN** show: the scheduler's key, reliably and continuously, because its lock is sticky for the goroutine's lifetime — all eight W2-1 censuses returned exactly that one row, and it is the right instrument for identifying the scheduler leader and for proving failover left nothing orphaned. What `pg_locks` **CANNOT** show: the other ten keys. Their hold is ~1ms (median 0.0010s, max 0.0100s) out of a 30s-to-1h tick, i.e. a duty cycle around 3×10⁻⁵, so a point-in-time census almost never catches one and **an empty row is the expected result, not evidence the job is dead** — W2-1 recorded 15,665 lock events over the same period in which every census showed a single row. For the per-tick keys use **Postgres statement logging** instead (`ALTER SYSTEM SET log_statement='all'` + `log_line_prefix='%m [%p] h=%h '`, reverted at teardown), which yields a complete timestamped per-replica record of every `pg_try_advisory_lock`/`pg_advisory_unlock` with its bind parameter; `log_parameter_max_length` is `-1` on `postgres:16-alpine` so the `DETAIL: parameters: $1 = '<key>'` line is emitted for free. Cost: ~40MB of Postgres log per 26 minutes on an idle 3-replica stack (~0.9MB gzipped). The alternative is to make the job produce work so its `if n > 0` summary line fires.
- **CORRECTED after W2-1 execution — audit retention IS wired by default; it does not require a flag.** The table above said "no-op unless `--audit-retention-days > 0`". `auditRetentionDaysDefault()` (`cmd/controller/main.go:26-41`) returns **90** when `UNIFIED_AUDIT_RETENTION_DAYS` is unset, and `main.go:415-420` logs `"audit log retention enabled"` and then starts `RunAuditRetention` **unconditionally** (the `retentionDays <= 0` guard inside `audit_retention.go:22-24` is therefore never hit on a default deployment). All three `test/ha` controllers boot-logged `"audit log retention enabled" retentionDays=90` (`w2-1/controller-boot.txt:2,8,13`). Its first tick is at boot+1h, so a sub-hour scenario observes nothing — that is a **null result, not a defect**. Generally: a key with zero holders is three different results — not wired, wired but not yet ticked, wired but broken — and only the third is a finding. Check the controller boot log first; the un-wired jobs announce themselves (`"no object store configured — log archival disabled"`, `"run retention disabled (keep forever)"`, `"log trim disabled (DB log rows kept forever)"`).
- **ESTABLISHED by W2-1 execution — `docker compose stop <agent>` CANNOT age out an agent, and this breaks Task 4's Arm C as written.** A healthy agent's SIGTERM path deregisters itself: after ctx cancel it builds a **fresh 5s context** and calls `Deregister` (`internal/agent/agent.go:343-350`), issuing `DELETE FROM agents WHERE id = $1` and logging `"agent deregistered"`. W2-1 measured the row gone **0.9s** after `stop` (`16:07:05.014Z` stop → `16:07:05.319` DELETE → row absent at the `16:07:05.943Z` sample, `w2-1/stale-agent-timeline.txt`), so `last_seen_at` never ages and `DeleteStaleAgents` is never exercised at all. **Any scenario that needs a stale `last_seen_at` must use `kill -s SIGKILL` (`inject.sh kill-hard`) or an nginx block instead**, and must start the threshold clock from the agent's **last heartbeat** (`agents.last_seen_at`), not from the kill. **This directly affects Task 4 (W2-3) Arm C**, which plans to "keep the agent blocked past 5 minutes so its row is deleted" in order to reach `DeleteStaleAgents`' 5-minute threshold: if that arm is executed with `stop`, the row disappears in under a second by a completely different code path and the arm silently measures nothing. Use `kill-hard` or `nginx-block` there. (This deregistration behaviour is itself undocumented — `grep -rn -i deregister docs/` matches nothing — but W2-1 recorded it as a carry-forward note rather than filing it.)
...
- **Three distinct orphan definitions — do not conflate:** startup reconcile `claimed_by=$1 AND status='Running'` with **no grace** (`ListRunningRunIDsByAgent`, `postgres.go:266-285`, bare SELECT at `:271`); heartbeat reconcile adds `claimed_at < NOW()-60s` and subtracts the reported set (`ListReconcilableRunIDsByAgent`, `postgres.go:287-309`, predicate at `:294`; `api_agent.go:101-119`, `heartbeatReconcileGrace = 60s` at `:71`); stuck-run reaper as above. **Do not cite `:294` for the ungraced query — that line is the *graced* one, and W2-2 shipped a draft with the two inverted.**
- **ESTABLISHED by W2-2 execution — bare `docker compose restart` gives ~1.013 s before SIGKILL, NOT 10 s.** Measured directly with `docker events` on the `test/ha` rig (`w2-2/partB-docker-events.txt:7,11,13`): `kill sig=15` at epoch `1785345889.394449855`, `kill sig=9` at `1785345890.406859166`, `die exit=137`. Docker's *documented* default stop timeout is 10 s; the observed value is an order of magnitude smaller and **the cause is not established** (no `stop_grace_period` in `test/ha/docker-compose.ha.yaml`, no further instrumentation run). **Consequence for later tasks: any arm that uses a bare `restart` to mean "graceful replacement" is really testing SIGKILL-after-~1 s.** If an arm needs a real drain it must pass `-t <seconds>` explicitly; if it needs a hard kill, `restart` is adequate but say so rather than calling it graceful. Affects any W2/W3 arm that restarts an agent or a controller.
- **ESTABLISHED by W2-2 execution — the agent drain is real and effectively unbounded, and a drained agent leaves nothing to reconcile.** With `docker compose stop -t 200`, `agent1` held an in-flight `edge-sideeffect` run for **107.311 s** past SIGTERM (SIGTERM `17:21:50.645653Z` → run `Succeeded 17:23:37.956706`), then logged `"agent deregistered"` and exited 0 (`w2-2/partB2-drain.txt`). On the way back up the startup reconcile found **nothing**: `w2-2/final-sweep.txt:40-43` holds exactly three `agent reconcile: failed orphaned run` lines across the whole scenario, one per restart-based arm and **none for the drained arm**. **Consequence for later tasks: "an agent shutdown lost work" is only a finding if the shutdown was *not* drained — use a `stop -t <long>` arm as the control before filing one.** This retires the plan's original "rolling restart kills in-flight work with no drain window" premise (see Task 3, corrected below).
- **CORRECTED after W2-2 execution — `cancelDescendantRuns` has FOUR direct call sites, and the five listed above are not them.** The list below (stuck-run reaper `:58`, heartbeat reconcile `api_agent.go:114`, startup reconcile `:824`, queued-run reaper `queuedrun_reaper.go:77`, claim-build failure `api_agent.go:188`) is really `failOrphanedRun`'s four callers plus the queued-run reaper, and it **omits two real entry points**. Verified by grep at W2-2 execution: the direct call sites are `api_runs.go:382` (public cancel endpoint), `api_agent.go:626` (the agent's own parent-run-finished report), `queuedrun_reaper.go:77`, and `stuckrun_reaper.go:88` (inside `failOrphanedRun`). Expanding `failOrphanedRun`'s callers gives **seven entry points** in total. This matters for W2-5's "blast radius" bullet (Task 6, which says "all five `cancelDescendantRuns` call sites") and for the W2-2 dangling-step finding: **none** of the seven terminalizes a descendant's step rows, because PR #113 put `MarkRunStepsInterrupted` inside `failOrphanedRun` scoped to its own `runID` only.
...
- [ ] **Step 5: Author `test/edgecase/tools/bulk-submit.sh`** — POSIX `sh`, `set -eu`, same style as `inject.sh`. Usage `bulk-submit.sh <job-name> <count>`; POSTs `count` triggers to **`POST /api/v1/runs` with body `{"jobName":"<job>"}`** (`internal/controller/server.go:370` → `handleTriggerRun`; **there is no `/api/v1/jobs/<job>/trigger` route** — an earlier draft of this plan said there was) against `${UNIFIED_SERVER:-http://localhost:18080}` with `${UNIFIED_TOKEN:-ha-admin-token}`, printing each run id one per line so the caller can `tee` it. Must be usable for W2-9's >50-run submission.
...
- [ ] **Step 1: Write the runbook.**
  ...
  - Recording: a run left `Running` with no agent = major (I1). A descendant left Running after its parent is failed = major (I3/I1). Immediate failure of a just-claimed run = judge on the documented contract: `postgres.go:271` has no grace by construction, so this is as-designed — likely **observation**, but state the operational cost prominently. **CORRECTED after W2-2 execution: the parenthetical this note originally carried — "a rolling agent restart kills in-flight work with no drain window" — is FALSE and must not be repeated.** The drain window exists, is documented (`--drain-timeout`, default 0 = wait forever) and was measured holding a run for 107.311 s past SIGTERM; what kills in-flight work is the *orchestrator's* stop grace truncating that drain into the ungraced startup reconcile. State it that way. Any arm asserting a shutdown lost work must run a `stop -t <long>` control first.
...
- [ ] **Step 1: Write the runbook.**
  ...
  - **Also record the blast radius:** the unlinked child is invisible to all four `cancelDescendantRuns` call sites — `api_runs.go:382`, `api_agent.go:626`, `queuedrun_reaper.go:77`, `stuckrun_reaper.go:88` (seven entry points once `failOrphanedRun`'s callers are expanded; **not** the five this plan originally listed — corrected in the facts block) — and to the WebUI caller→child navigation that `callstep.go:56-61` exists to enable.
