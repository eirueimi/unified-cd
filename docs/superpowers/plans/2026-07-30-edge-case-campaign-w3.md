...
- **Terminal-but-existing runs still accept uploads** (`api_artifacts.go:59-61`; the comment as a whole spans `:57-61` and its first half is the orphan warning), which is what makes the scenario constructible. **CONFIRMED LIVE BY W3-6.** But note what the plan did not say and Task 5 had to discover: **no *ordinary* run can supply the precondition**, because `DELETE /api/v1/runs/{id}` needs a terminal run (409 otherwise) while an agent-driven run is `Running` for the whole of its `uploadArtifact` step. W3-6 built a curl-driven **synthetic agent** (`test/edgecase/workloads/w36-probe.yaml`, `agentSelector: [kind:w36probe]`, enroll → claim → finish → upload; five API calls, no SQL) to get one.
