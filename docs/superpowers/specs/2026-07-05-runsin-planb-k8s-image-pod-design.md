...
- `internal/k8sagent/agent.go`:
  ...
  - `runImageStep` メソッド追加。stdout/stderr writer は既存 stepExec（167-174）の書き方（`logLineWriter` ＋ `stderrPusher` ＋ masker）を再利用し、共有 pod パスと同一のマスキング/ログ経路を通す。
...
- **出力:** stdout は `ExecStep` の stdout writer で捕捉し、既存どおり出力テンプレート（`outputs:`）に供給。stderr は log pusher でストリーム。両者とも既存の **secret masker** を通す。exit code は `ExecStep` 返り値。
