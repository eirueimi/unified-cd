...
- `.air.agent.toml`: this is the only air config with shim processing
  (`.air.toml` and `.air.controller.toml` are controller-only — **no change**).
  The current `[build].cmd` is a workaround for the git-ignored/placeholder
  scheme: it copies `/app` to a disposable tree (excluding the shim files),
  runs `prepare-shim-placeholders.sh`, builds a host-arch `ucd-sh`, then builds
  the agent. With committed real bytes, **all of that is obsolete** — the
  committed linux shim is already in the mounted worktree and the dev
  container's `GOARCH` selects the right arch via build tag. Replace the whole
  `[build].cmd` with a direct build:
  ...
  Keep the remaining `[build]` keys (`delay`, `include_ext`, `exclude_dir`,
  `kill_delay`, `rerun`, `poll`, `poll_interval`) and the other sections as-is.
  Rewrite the leading comment: it no longer needs to keep generated bytes out of
  the worktree (they belong there now). Per the **no-auto-regen** decision, air
  does **not** run `go generate`; a dev editing `cmd/ucd-sh` / `internal/shim`
  runs `go generate ./...` manually to refresh the committed shim before the
  agent picks it up — same convention as editing DSL types then running
  `schemagen`.
