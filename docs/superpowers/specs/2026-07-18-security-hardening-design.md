...
- Hash the on-disk shim against the `go:embed` bytes and rewrite it if it differs — at agent startup **and immediately before each claim's container-creating path**. This detects and repairs tampering left by an earlier run before the next claim starts, so it cannot persist indefinitely across the agent's lifetime, while fully respecting the shared-mount invariant. It does **not** bound tampering to within a single run when `MaxConcurrent > 1`: every slot shares the same `toolsDir`, and the `/.ucd` mount each claim pod gets is a **live** read-only bind mount of that same host directory (not a copy taken at pod start), with `ucd-sh` re-invoked for the pod's whole lifetime (pause keep-alive plus every step exec). A native step in one slot can tamper with the on-disk shim mid-run and an isolated job already running in another slot will pick up the tampered bytes on its very next `ucd-sh` invocation — before the next per-claim check fires, because the check only runs at the start of a new claim, never against claims already in flight.
...
**Residual / follow-up (out of scope for this wave):** the concurrent-slot window above is not closed by this design — closing it needs a check that runs independently of claim cadence, e.g. a periodic re-verify loop, or verifying shim integrity before each container-step exec rather than once per claim. Deliberately deferred; tracked as follow-up work, not a defect in this change.
