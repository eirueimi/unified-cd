...
unified-cd-agent [FLAGS]
  ...
  --finally-timeout       duration  Ceiling on each of a run's post-DAG cleanup phases (the spec.finally pipeline, and each post:/cache: hook drain); 0/unset = default 10m (env: UNIFIED_AGENT_FINALLY_TIMEOUT)
...
| `UNIFIED_AGENT_FINALLY_TIMEOUT` | Go duration bounding each of a run's post-DAG cleanup phases: the [`spec.finally`](../user-guide/writing-jobs/approval-and-finally.md#finally-block-finally) pipeline, and each `post:`/`cache:` hook drain (env equivalent of `--finally-timeout`). Unset or non-positive → **10m**; an unparseable value here is ignored (leaving 10m), while an unparseable `--finally-timeout` flag or `finallyTimeout:` config key is a startup error. These phases deliberately survive run cancellation, which also discards the job-level `spec.timeoutMinutes` deadline — this is the ceiling that replaces it. Four phases carry it (each hook drain, the `finally` pipeline, and scope/claim-pod teardown), so the worst-case post-DAG time is **4 ×** this value. The k8s agent's equivalent is `finallyTimeout` / `UNIFIED_K8S_FINALLY_TIMEOUT`; the behaviour is identical on both agents because the cleanup phases run in the shared orchestration loop — with one difference in the arithmetic, not the semantics: the k8s agent adds a fifth window (its claim Pod is deleted or pooled after that loop returns), where the standard agent tears its claim pod down inside the fourth. |
...
drainTimeout: 60s
finallyTimeout: 10m            # ceiling on each post-DAG cleanup phase (spec.finally, and each post:/cache: drain); default shown
...
pod](../user-guide/writing-jobs/isolation-and-containers.md#job-isolation-native-and-the-claim-pod)); the standard agent
...
| `sidecarS3SecretName` | string | No | — | Secret carrying `UNIFIED_S3_*` for the artifact sidecar. **The Secret must exist in the namespace `namespace` names** (where job Pods run), not the agent's own namespace — it is attached with a `LocalObjectReference`, which Kubernetes always resolves in the Pod's namespace. Unset: cache steps are no-ops (Succeeded, with one per-run warning) and a claim carrying an artifact step that would run unconditionally is failed immediately, before a Pod is created (`continueOnError` steps are exempt; `if:`-guarded ones only warn). Set but missing in the job namespace: **every** job fails, because `envFrom.secretRef` is not `optional` and the kubelet cannot start the Pod (`CreateContainerConfigError`) |
...
| `maxDetachedConcurrent` | int | No | `16` | Max simultaneous detached (`spec.detached`) runs, from a pool separate from `maxConcurrent`. `0`/unset → **16** (detached jobs are claimable out of the box); a **negative** value disables detached hosting on this agent; a positive value is the cap (env: `UNIFIED_K8S_MAX_DETACHED`). See [Detached runs](../user-guide/writing-jobs/templates-and-reuse.md#detached-runs-detached). |
...
| `finallyTimeout` | string | No | `10m` | Go duration bounding **each** of a run's post-DAG cleanup phases: the [`spec.finally`](../user-guide/writing-jobs/approval-and-finally.md#finally-block-finally) pipeline, and each `post:`/`cache:` hook drain. Env override: `UNIFIED_K8S_FINALLY_TIMEOUT`. Those phases deliberately keep running after a cancellation — that is what `finally:` is for — which also discards the job-level `spec.timeoutMinutes` deadline, so without this ceiling a cleanup step that never returns (most easily a `call:` waiting on a child run that will never be claimed) pinned the run and the agent's concurrency slot indefinitely. Unset or non-positive falls back to the default; "unbounded" is deliberately not expressible. An **unparseable** value is rejected at startup (`invalid config: finallyTimeout "10 minutes": …`) and the agent exits 1 — it does not fall back. It is a per-phase budget, not a shared total: four phases carry it inside the shared cleanup loop (each hook drain, the `finally` pipeline, and scope/Pod teardown), and this agent adds a fifth after that loop returns — the claim Pod's own deletion or return to the idle pool. So the worst-case post-DAG time here is **5 × this value**, 50m at the default; size a rollout's `terminationGracePeriodSeconds` against that, not against the standard agent's 4 × / 40m. Per-step `timeoutMinutes:` works inside `finally:` and remains the precise tool. Parity with the standard agent's `--finally-timeout`. |
...
`UNIFIED_K8S_POD_START_TIMEOUT`, `UNIFIED_K8S_DRAIN_TIMEOUT`, and
`UNIFIED_K8S_FINALLY_TIMEOUT` override their config fields.
