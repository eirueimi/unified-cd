...
| `unvalidated` | bool | No | Explicit opt-out of the `pattern` requirement for a webhook payload-mapped param (see [WebhookReceiver](../resources/webhook-receiver.md#webhookreceiver)). Use only when the value is genuinely free-form and never reaches a shell. |
