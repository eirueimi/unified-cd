...
1. **`docs/*.md`** — grep for every field/flag/behavior you added, renamed, or removed. Beyond the obvious reference pages (`docs/user-guide/writing-jobs/job-structure.md`, `docs/user-guide/resources/job.md`, `docs/operator-manual/agents.md`, `docs/reference/configuration.md`, `docs/operator-manual/kubernetes-integration.md`), always check the narrative docs: **`docs/getting-started/quickstart.md`** (do the quickstart steps still work end-to-end for a new user?), **`docs/troubleshooting/controller-and-database.md`** (add symptom entries for any new failure mode, with the exact grep-able error string), and **`docs/operator-manual/operations.md`** (any new operator responsibility, e.g. disk/container hygiene).
2. **Generated artifacts** — `docs/reference/field-reference.md` and `schemas/unified-cd.schema.json` are BOTH generated; never hand-edit either. A new root resource kind must be added to `cmd/schemagen/main.go`'s `roots` AND `cmd/docgen/main.go`'s `rootKinds`, and its type definitions must live in `internal/dsl/types.go` or a `*_types.go` file (schemagen scans only those). Then run `go generate ./...` and commit the diff.
...
5. **Breaking changes** — add or update a `docs/operator-manual/migrations/*.md` guide with a before→after table and the exact validation-error strings users will see.
