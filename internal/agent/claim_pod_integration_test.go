...
// failing — when internal/shim/embedded still holds an empty payload (the
// committed internal/shim/embedded/ucd-sh-<arch> bytes, generated by `go
// generate`, are missing or empty), mirroring the crt.Detect "no runtime"
// skip pattern used throughout this file.
func installShimOrSkip(t *testing.T) string {
	t.Helper()
	toolsDir, err := InstallShim(t.TempDir())
	if err != nil {
		t.Skipf("ucd-sh shim not embedded (run `go generate ./internal/shim/embedded/` first): %v", err)
	}
	return toolsDir
}
