...
func TestEnsureShimIntact_RepairsTamperedShim(t *testing.T) {
	...
	// repair path regardless of whether internal/shim/embedded holds real
	// committed shim bytes or an empty payload (see withFakeShimBytes's doc
	// comment in install_shim_test.go) — the same convention that package's
	// InstallShim tests already use, rather than skipping and losing this
	// coverage in every CI run.
...
func TestEnsureShimIntact_NoopWhenIntact(t *testing.T) {
	...
	// bytes so this runs the same regardless of the committed shim bytes in internal/shim/embedded.
	withFakeShimBytes(t, []byte("#!/bin/sh\necho real-shim-content\n"))
	toolsDir, err := InstallShim(ws)
	require.NoError(t, err)
...
func TestEnsureShimIntact_EmptyPayloadIsHardError(t *testing.T) {
	...
	// internal/shim/embedded holds the committed
	// internal/shim/embedded/ucd-sh-<arch> bytes, generated by `go
	// generate`, or an empty payload — it is the one test in this file that
