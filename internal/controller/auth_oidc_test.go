...
func TestHandleMe_NoCookie(t *testing.T) {
	...
	srv := NewServer(Config{Token: "t", AgentToken: "t"}, pg)
...
func TestHandleMe_ValidSession(t *testing.T) {
	...
	_, err := pg.CreateSession(context.Background(), tokenHash, "sub123", "test@example.com", "encrypted-rt", expiresAt)
	require.NoError(t, err)
...
func TestHandleLogout_ClearsSession(t *testing.T) {
	...
	tokenHash := HashToken(sessionToken)
	_, err := pg.CreateSession(context.Background(), tokenHash, "sub456", "logout@example.com", "enc-rt", time.Now().Add(time.Hour))
	require.NoError(t, err)
