...
func (s *Server) agentRunGuard(ctx context.Context, agentID, runID string, rejectTerminal bool) (runWriteVerdict, error) {
	if !rejectTerminal {
		if owner, ok := s.claimedBy.get(runID); ok {
			...
			// Record the policy-denial metric for every principal: a denied
			// write is still a denied write and must show up in the same
			// signal every bearer credential does.
