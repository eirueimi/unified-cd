...
func TestExpandUses_PropagatesContainer(t *testing.T) {
	...
	expanded, err := expandUsesStep("tmpl", nil, tplSpec, nil, "builder")
...
func TestInline_TemplateStepRunsInRejected(t *testing.T) {
	...
	_, err := expandUsesStep("build", nil, templateWithInnerRunsIn, nil, "")
	require.Error(t, err)
...
func TestInline_UsesContainerPropagatesToInnerSteps(t *testing.T) {
	...
	expanded, err := expandUsesStep("job", nil, tplSpec, nil, "tools")
	require.NoError(t, err)
