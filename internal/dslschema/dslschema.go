// Package dslschema parses internal/dsl/*_types.go and builds the JSON
// Schema representation of it. It is shared by cmd/schemagen — which
// writes the result to schemas/unified-cd.schema.json — and by
// cmd/docgen's freshness test, which needs the same in-memory schema
// without going through that committed file, so it can catch a stale
// docs/reference/field-reference.md directly against internal/dsl even in
// a hypothetical run where schemas/unified-cd.schema.json was hand-edited
// to match. Keeping the parsing/building logic in one importable package
// (instead of copied into cmd/docgen's test) means both call sites always
...
// BuildSchema builds the full JSON Schema document from parsed struct info.
