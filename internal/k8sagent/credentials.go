...
const kubernetesCredentialRetryAttempts = 3
...
func NewKubernetesCredentialSource(cfg KubernetesCredentialSourceConfig) *KubernetesCredentialSource {
	...
	return &KubernetesCredentialSource{
		...
		http: httpClient, now: now, sleep: sleepKubernetesCredential,
	}
}

// Token returns the active access token, exchanging the current projected JWT
// only when the previous access token has expired.
func (s *KubernetesCredentialSource) Token(ctx context.Context) (string, error) {
	s.mu.Lock()
	defer s.mu.Unlock()
	if s.accessToken != "" && s.now().Before(s.accessExpiresAt) {
		return s.accessToken, nil
	}

	projectedToken, err := readProjectedServiceAccountToken(s.serviceAccountTokenFile)
	if err != nil {
		return "", err
	}
	response, err := s.exchangeWithRetry(ctx, projectedToken)
	if err != nil {
		return "", err
	}
...
func (s *KubernetesCredentialSource) exchangeWithRetry(ctx context.Context, token string) (api.AgentTokenResponse, error) {
...
func (s *KubernetesCredentialSource) exchange(ctx context.Context, token string) (api.AgentTokenResponse, error) {
	...
	if resp.StatusCode < http.StatusOK || resp.StatusCode >= http.StatusMultipleChoices {
		return response, &kubernetesExchangeError{retryable: resp.StatusCode == http.StatusTooManyRequests || resp.StatusCode >= http.StatusInternalServerError}
	}
