...
//
// Each member emits two stdout and two stderr lines, drops a signal file
// named after its own index into the shared /workspace, then spins waiting
// for every other member's signal file to appear before emitting two more
// stdout and two more stderr lines and exiting 0.
//
// This is deliberately NOT a timing assertion (the design brief calls that
// out as flaky-by-construction): nothing here measures wall-clock overlap or
// finish order. Instead it is a rendezvous every member must clear before any
// of them can finish. If Task 2's ConcurrencyMode flip regressed and the k8s
// backend went back to running parallel members one at a time, member 0 would
// reach the wait loop while members 1 and 2 haven't started yet — their
// signal files would never appear, member 0 would exhaust the bounded retry
// loop below, and the step would fail loudly instead of the suite hanging.
// The 150*0.2s = 30s bound is generous versus real concurrent-exec startup
// latency on a kind cluster, but still finite so a regression fails cleanly
// well inside the test's overall timeout instead of blocking CI.
...
while ! ( %[2]s ); do
  ...
  if [ "$attempt" -ge 150 ]; then
    echo "member-%[1]d: gave up waiting for the other members' signal files after 30s -- steps did not run concurrently" >&2
    exit 1
  fi
  sleep 0.2
done
...
// three independent StepLogWriters auto-flush timers, and the single
// per-claim sidecar log pump running beneath all of it. See
// concurrentBarrierScript for why the concurrency proof is a deterministic
// rendezvous rather than a timing measurement.
//
...
// actually arrived at the controller (proving the per-step log writers and
// the shared sidecar pump held up under concurrent load). Nothing here
// asserts ordering or measures elapsed time.
func TestK8sAgent_ExecuteRun_ParallelMembersRunConcurrently_Integration(t *testing.T) {
	...
	mux := http.NewServeMux()
	mux.HandleFunc("POST /api/v1/agents/"+agentID+"/register", func(w http.ResponseWriter, _ *http.Request) {
		w.WriteHeader(http.StatusNoContent)
	})
